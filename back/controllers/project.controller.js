...
const access = require("node:fs").access
...
module.exports.createProject = (req, res) => {
  const newProject = new project_models({
    ...
    picture: req.file
      ? `./uploads/projects/${req.body.picture}`.split(" ").join("")
      : "",
  })
...
module.exports.readProjects = (req, res) => {
  project_models
    .find({creatorId: req.params.id},(err, docs) => {
...
module.exports.deleteProject = (req, res) => {
  project_models.findByIdAndRemove(req.params.id, (err, docs) => {
    if (!err) res.send(docs)
